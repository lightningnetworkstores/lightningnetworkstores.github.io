...
const actions = {
    ...
    getStoreVotePaymentRequest(
        ...
        return fetch(
            `${state.baseURL
            }api/get_invoice?amount=${amount}&storeID=${id}&direction=${isUpvote ? 'Upvote' : 'Downvote'
            }${comment ? '&comment=' + comment : ''}${parent ? '&parent=' + parent : ''
    ...
    getDiscussionReplyPaymentRequest({ state }, payload) {
        return this.$axios
            .post(
                `${state.baseURL}api/discussion${payload.recaptchaToken

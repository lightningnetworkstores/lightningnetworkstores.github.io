      ...
      <v-expansion-panels>
        ...
        >
          <v-expansion-panel-header
            class="d-flex flex-column align-end my-0 pt-0 pb-2 px-3"
...
export default {
  ...
  computed: {
    headers() {
      console.log(
        ':: => :: ',
        this.filteredDiscussions(this.selectedTopic).map(
          (comments) => comments[0]
        )
      )
      return this.filteredDiscussions(this.selectedTopic).map(
        (comments) => comments[0]
      )
    },
    replies() {
      return (index) =>
        this.filteredDiscussions(this.selectedTopic)[index].slice(1)
    },
    repliesCount() {
      return (index) =>
        this.filteredDiscussions(this.selectedTopic)[index].length - 1
    },
    threadId() {
      return (index) =>
        this.filteredDiscussions(this.selectedTopic)[index][0].thread_id
    },
